...
const (
	testRepoBaseName = "git-bug-test-gitea-exporter"
)
...
func TestPushPull(t *testing.T) {
	...
	require.NoError(t, err)
	author.SetMetadata(metaKeyGiteaLogin, login)
	err = author.Commit()
	require.NoError(t, err)
	...
	err = exporter.Init(ctx, backend, core.Configuration{
		...
		confKeyGiteaBaseUrl: defaultBaseURL,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.name == "bug changed status" {
				t.Skip("test known as broken, see https://github.com/MichaelMure/git-bug/issues/435 and complain to gitea")
			...
			for _, op := range tt.bug.Snapshot().Operations {
				...
				if _, ok := op.(*bug.SetMetadataOperation); !ok {
					_, haveIDMetadata := op.GetMetadata(metaKeyGiteaId)
					require.True(t, haveIDMetadata)

					_, haveURLMetada := op.GetMetadata(metaKeyGiteaUrl)
					require.True(t, haveURLMetada)
				}
			}

			// get bug gitea ID
			bugGiteaID, ok := tt.bug.Snapshot().GetCreateMetadata(metaKeyGiteaId)
			require.True(t, ok)

			// retrieve bug from backendTwo
			importedBug, err := backendTwo.ResolveBugCreateMetadata(metaKeyGiteaId, bugGiteaID)
			require.NoError(t, err)

			// verify bug have same number of original operations
			require.Len(t, importedBug.Snapshot().Operations, tt.numOpImp)

			// verify bugs are tagged with origin=gitea
...
func deleteRepository(ctx context.Context, project int, token *auth.Token) error {
	...
	_, err = client.Projects.DeleteProject(project, gitea.WithContext(ctx))
	return err
}

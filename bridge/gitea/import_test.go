...
func TestImport(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "simple issue",
			url:  "https://gitea.com/git-bug/test/-/issues/1",
			bug: &bug.Snapshot{
		...
		{
			name: "empty issue",
			url:  "https://gitea.com/git-bug/test/-/issues/2",
			bug: &bug.Snapshot{
				Operations: []bug.Operation{
		...
		{
			name: "complex issue",
			url:  "https://gitea.com/git-bug/test/-/issues/3",
			bug: &bug.Snapshot{
				Operations: []bug.Operation{
		...
		{
			name: "editions",
			url:  "https://gitea.com/git-bug/test/-/issues/4",
			bug: &bug.Snapshot{
				Operations: []bug.Operation{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			b, err := backend.ResolveBugCreateMetadata(metaKeyGiteaUrl, tt.url)
			require.NoError(t, err)

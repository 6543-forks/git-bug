...
func (g *Gitea) Configure(repo *cache.RepoCache, params core.BridgeParams) (core.Configuration, error) {
	var err error
	var baseUrl string
	...
	default:
		baseUrl, err = input.PromptDefault("Gitea server URL", "URL", defaultBaseURL, input.Required, input.IsURL)
		if err != nil {
	...
	// validate project url and get its owner and name
	pOwner, pName, err := validateProjectURL(baseUrl, projectURL, token)
	if err != nil {
	...
	conf[confKeyOwner] = pOwner
	conf[confKeyProject] = pName
	conf[confKeyGiteaBaseUrl] = baseUrl
...
func (g *Gitea) ValidateConfig(conf core.Configuration) error {
	...
	if _, ok := conf[confKeyGiteaBaseUrl]; !ok {
		return fmt.Errorf("missing %s key", confKeyGiteaBaseUrl)
	}
...
func promptProjectURL(repo repository.RepoCommon, baseUrl string) (string, error) {
	validRemotes, err := getValidGiteaRemoteURLs(repo, baseUrl)
	if err != nil {
		return "", err
	}

	return input.PromptURLWithRemote("Gitea project URL", "URL", validRemotes, input.Required)
}

func getProjectPath(baseUrl, projectUrl string) (string, string, error) {
	cleanUrl := strings.TrimSuffix(projectUrl, ".git")
	split := strings.SplitN(cleanUrl, "@", 1)
	if len(split) == 2 {
		cleanUrl = "https://" + strings.ReplaceAll(split[1], ":", "/")
	}
	...
	split = strings.Split(objectUrl.Path, "/")
	if len(split) != 2 {
		return "", "", ErrBadProjectURL
	}
...
func getValidGiteaRemoteURLs(repo repository.RepoCommon, baseUrl string) ([]string, error) {
	...
	for _, u := range remotes {
		pOwner, pName, err := getProjectPath(baseUrl, u)
		if err != nil {
			continue
		}

		urls = append(urls, fmt.Sprintf("%s/%s/%s", baseUrl, pOwner, pName))
	}

	return urls, nil
}

func validateProjectURL(baseUrl, url string, token *auth.Token) (string, string, error) {
	pOwner, pName, err := getProjectPath(baseUrl, url)
	if err != nil {
		return "", "", err
	}
	...
	project, _, err := client.GetRepo(pOwner, pName)
	if err != nil {
		return "", "", errors.Wrap(err, "wrong token scope ou non-existent project")
	}

	return project.Owner.UserName, project.Name, nil
}
